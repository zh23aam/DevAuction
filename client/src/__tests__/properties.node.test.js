...
/**
 * Property-Based Tests for Frontend Features (Node.js compatible)
    ...
    fc.assert(
      fc.property(
        ...
        (scenario) => {
          ...
          // Verify total count
          if (allBidsFromPages.length !== totalBids) {
            throw new Error(`Expected ${totalBids} bids, got ${allBidsFromPages.length}`);
          }

          // Verify no duplicates
          const bidIds = allBidsFromPages.map((b) => b.id);
          const uniqueIds = new Set(bidIds);
          if (uniqueIds.size !== totalBids) {
            throw new Error('Duplicate bids found');
          }
    ...
    fc.assert(
      fc.property(
        ...
        (messages) => {
          ...
          const uniqueIds = new Set(messageIds);
          if (uniqueIds.size !== messageIds.length) {
            throw new Error('Message appears in multiple auctions');
          }
        }
      ),
      { numRuns: 100 }
