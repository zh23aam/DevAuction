...
const Followering = ({ resp, Data, showFollowing, setShowFollowing }) => {
  ...
  const fetchFollowing = useCallback(async () => {
    const followingList = Data?.userData?.Profile?.Following || [];
    ...
    setIsLoading(true);
    try {
      const result = await api.post("/profile/followers", { followers: followingList });
      setFollowing(result.data || []);
    } catch (err) {
      console.error("Fetch Following Error:", err);
      showToast("Failed to load following list", "red");
    } finally {
      setIsLoading(false);
    }
  }, [Data?.userData?.Profile?.Following, showToast]);

  useEffect(() => {
    fetchFollowing();
  }, [fetchFollowing]);

  const unfollowMe = async (targetEmail, name) => {
    ...
    try {
      ...
      showToast(`Unfollowed ${name}`, "blue");
      if (resp) resp();
    } catch (err) {
      console.error("Unfollow Error:", err);
      showToast(`Failed to unfollow ${name}`, "red");
    } finally {
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6 pt-2">
        {isLoading ? (
          ...
          <EmptyState 
            size="sm"
            icon="🤝"
            title={searchTerm ? "No match" : "Not following"}
            description={searchTerm ? "Try a different name" : "You're not following anyone yet"}
          />
        ) : (
          <div className="space-y-3">
            {filteredFollowing.map((person) => (
              <div key={person.email} className="p-3 flex items-center justify-between border-b border-white/5 hover:bg-white/5 rounded-xl transition-colors group">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white/10 shadow-lg group-hover:border-[#0CA3E7]/50 transition-colors">
                    <img src={person.image || profileImg} alt={person.name} className="w-full h-full object-cover" />
                  </div>
                  <span className="font-semibold text-gray-200">{person.name}</span>
                </div>
                <button 
                  disabled={isActionLoading}
                  onClick={() => unfollowMe(person.email, person.name)}
                  className="px-4 py-1 text-xs font-bold uppercase tracking-widest bg-[#0CA3E7] hover:bg-[#0A8BC7] text-white rounded-full transition-all active:scale-95 disabled:opacity-50"
                >
                  Unfollow
                </button>
              </div>
